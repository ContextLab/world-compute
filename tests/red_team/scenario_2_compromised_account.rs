...
fn compromised_manifest() -> worldcompute::scheduler::manifest::JobManifest {
    ...
    worldcompute::scheduler::manifest::JobManifest {
        manifest_cid: None, name: "normal-job".into(),
...
fn attack_2a_banned_account_cannot_submit() {
    let ctx = SubmissionContext {
        submitter_peer_id: "12D3KooWBanned".into(), submitter_public_key: vec![0; 32],
        submitter_hp_score: 10, submitter_banned: true,
...
fn attack_2b_zero_hp_account_cannot_submit() {
    let ctx = SubmissionContext {
        submitter_peer_id: "12D3KooWSybil".into(), submitter_public_key: vec![0; 32],
...
fn attack_2c_low_hp_cannot_vote_on_emergency_halt() {
    let proposal = GovernanceProposal {
        proposal_id: "p-halt".into(), title: "Halt".into(), body: "Emergency".into(),
        proposal_type: ProposalType::EmergencyHalt, state: ProposalState::Open,
        submitter_id: "alice".into(), created_at: Timestamp::now(),
        closes_at: Timestamp::now(), yes_votes: 0, no_votes: 0, abstain_votes: 0,
    };
    let vote = Vote {
        vote_id: "v1".into(), proposal_id: "p-halt".into(), voter_id: "compromised".into(),
        choice: VoteChoice::Yes, weight: 1, signature: vec![], cast_at: Timestamp::now(),
    };
...
fn attack_2e_quota_flooding_blocked() {
    let ctx = SubmissionContext {
        submitter_peer_id: "12D3KooWFlooder".into(), submitter_public_key: vec![0; 32],
        submitter_hp_score: 10, submitter_banned: false,
        epoch_submission_count: 1000, epoch_submission_quota: 100,
    };
    let d = evaluate(&compromised_manifest(), &ctx).unwrap();
    assert_eq!(d.verdict, Verdict::Reject);
