...
fn attack_3a_forged_signature_rejected() {
    let d = evaluate(
        ...
        &SubmissionContext {
            submitter_peer_id: "12D3KooWForger".into(),
...
fn attack_3d_separation_of_duties_violation_blocked() {
    let existing = vec![GovernanceRole::new(
        "r1".into(),
        "attacker".into(),
        RoleType::WorkloadApprover,
        "admin".into(),
    )];
    let err =
        check_separation_of_duties("attacker", RoleType::ArtifactSigner, &existing).unwrap_err();
...
fn attack_3e_constitution_amendment_timelock_cannot_be_bypassed() {
    let mut proposal = GovernanceProposal {
        proposal_id: "p-bypass".into(),
        title: "Remove safety".into(),
        body: "Remove Principle I".into(),
        proposal_type: ProposalType::ConstitutionAmendment,
        state: ProposalState::Draft,
        submitter_id: "attacker".into(),
